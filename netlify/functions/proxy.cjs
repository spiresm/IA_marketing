...
                break;
            ...
            case 'deleteProfil': // NOUVEAU/MIS À JOUR : Gérer la suppression de profil
                targetUrl = `https://${event.headers.host}/.netlify/functions/deleteProfil`;
                fetchMethod = 'DELETE'; // LA MÉTHODE HTTP DOIT ÊTRE DELETE
                requestBody = event.body; // L'ID du profil est dans le corps de la requête
                break;
...
        // Ajoute le corps seulement s'il existe et si la méthode HTTP le permet (POST, PUT, PATCH, DELETE)
